...
from models import *
from queries import GET_ACTIONS_QUERY
...
async def get_actions(request: ActionsRequest):
    try:
        ...
        all_actions = tuple(request.actions + [f"end_{action}" for action in request.actions])
        ...
        for entity_id in request.entity_ids:
            ...
            actions_data = {action: [] for action in request.actions}
            total_duration = {action: 0.0 for action in request.actions}
            ...
            average_duration = {
                action: total_duration[action] / session_count[action] if session_count[action] > 0 else None
                for action in request.actions
            }
